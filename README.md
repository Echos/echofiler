...
- `←` / `Ctrl+h`: 左ペインにフォーカス
- `→` / `Ctrl+l`: 右ペインにフォーカス
- `o`: もう一方のペインと同じディレクトリへ移動（アクティブペインを変更）
- `O`: 自分のディレクトリをもう一方のペインにも適用（もう一方のペインを変更）
...
- `Enter`: ディレクトリに入る / ファイルを開く（デフォルトアプリケーション）
...
`q` で終了すると、左右ペインのディレクトリとアクティブなペインを記憶し、次回起動時に復元します。

- 記憶先: `${XDG_DATA_HOME:-~/.local/share}/echofiler/session.toml`
- 記憶したディレクトリが削除されている場合はカレントディレクトリから開始します
- 常にカレントディレクトリから開始したい場合は `echofiler.toml` で無効化できます
